...
# ==================================================
#  ДАННЫЕ ДОПОЛНИТЕЛЬНЫХ УСЛУГ (добавлены поля title)
# ==================================================
EXTRA_SERVICES = {
    'master_classes': [
        {
...
def get_extra_actions_keyboard():
    """Клавиатура для просмотра деталей дополнительной услуги"""
    keyboard = VkKeyboard(one_time=True)
    keyboard.add_button('✅ Хочу заказать', color=VkKeyboardColor.POSITIVE)
    keyboard.add_line()
    keyboard.add_button('📞 Связь с оператором', color=VkKeyboardColor.PRIMARY)
    ...
    if current_state == 'main':
        ...
        for service in services:
            ...
            if title and user_message == title.lower():
                ...
                break
        if selected_service:
            # Если это специальная кнопка "Не нашли программу?" (в доп. услугах такого нет, но на всякий случай)
            if selected_service.get('special'):
        ...
        elif user_message in ['◀ назад', 'назад']:
            # Возврат к списку услуг
            user_stack[user_id].pop()
            extra_cat = user_temp.get(user_id, {}).get('extra_category')
            if extra_cat:
